...
void run_benchmark(const std::string& op_name, int width, int height, int region_size, int iters) {
    ...
    if (op_name == "Fill Region") {
        auto op_vi = [&](int r, int c) { vi.fill_region(r, c, r + region_size - 1, c + region_size - 1, {0, 255, 0}); };
        auto op_st = [&](int r, int c) { st.fill_region(r, c, r + region_size - 1, c + region_size - 1, {0, 255, 0}); };
        time_vi = time_operation([&]() { for(const auto& reg : regions) op_vi(reg.first, reg.second); });
        time_st = time_operation([&]() { for(const auto& reg : regions) op_st(reg.first, reg.second); });
    } else if (op_name == "Adjust Brightness") {
        auto op_vi = [&](int r, int c) { vi.adjust_brightness(r, c, r + region_size - 1, c + region_size - 1, 20); };
        auto op_st = [&](int r, int c) { st.adjust_brightness(r, c, r + region_size - 1, c + region_size - 1, 20); };
        time_vi = time_operation([&]() { for(const auto& reg : regions) op_vi(reg.first, reg.second); });
        time_st = time_operation([&]() { for(const auto& reg : regions) op_st(reg.first, reg.second); });
    }
...
int main() {
    ...
    const std::vector<std::string> OPERATIONS = {"Fill Region", "Adjust Brightness"};
    const std::vector<int> REGION_SIZES = {4096, 1080, 64};
    const std::vector<int> ITERATIONS = {1, 1000, 100000};

    // CSV Header
    std::cout << "Operation,RegionSize,Iterations,VectorTime,TreeTime" << std::endl;

    for (const auto& op : OPERATIONS) {
        for (int region : REGION_SIZES) {
            for (int iter : ITERATIONS) {
                // Don't run huge iterations on huge regions
                if (region > 1000 && iter > 1000) continue;
                if (region > 64 && iter > 10000) continue;
