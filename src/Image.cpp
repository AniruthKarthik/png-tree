...
RGB_uc Image::get_pixel(int r, int c) const {
    ...
    for (int r = 0; r < height; ++r) {
        for (int c = 0; c < width; ++c) {
            if (r >= r1 && r <= r2 && c >= c1 && c <= c2) {
                ...
                blurred_image.set_pixel(r, c, {(unsigned char)(red / count), (unsigned char)(green / count), (unsigned char)(blue / count)});
            } else {
...
std::vector<std::vector<int>> Image::histogram() const {
    std::vector<std::vector<int>> hist(3, std::vector<int>(256, 0));
    for (int r = 0; r < height; ++r) {
        for (int c = 0; c < width; ++c) {
            RGB_uc pixel = get_pixel(r, c);
            hist[0][pixel.r]++;
            hist[1][pixel.g]++;
            hist[2][pixel.b]++;
        }
